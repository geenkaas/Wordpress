...
//	When user scrolls
	$(window).scroll(function() {
		...
		//	Check how far user is scrolled
			...
			var menuscrollmax = $('#sidemenu').offset().top;
			if (scrolled > menuscrollmax - 10) {
				$('#sidemenu').css({
					position: 'fixed',
					top: 10
				});
...
var mySliders = Array();
	...
	function geenSlider() {
		...
		//	Initialise timer
			...
			//slideInterval = setInterval('sliderPrep()', slideTimer);
			
			//smySliders[0].sliderPrep();
	};

function bbSlider(element, idx){
	...
	clearTimeout(slideInterval);
	var sf = 'mySliders[{0}].sliderPrep()'.format(idx);
	slideInterval = setInterval(sf, slideTimer);
	
	
	//	Set variables for every slider and send them to the slides. Once!
	function sliderInit() {
		var e = slider;
		//	Find all slides and set basic properties
